...
jQuery.extend({ debug : function(options) {
	...
	this.dump = function(x,label) {
		var myDate = new Date();
		var s ="["+ myDate.getHours()+':'+myDate.getMinutes()+":"+myDate.getSeconds()+"."+myDate.getMilliseconds()+"]";
	...
	this.out = function(s) {
		...
		var kk = $(this.Box).find("#debug_id2")[0]; 
		if (Prefs.continuous){ 
			jQuery(kk).prepend(s+Prefs.itemDivider);
		}else{
			jQuery(kk).html(s);
		}
	}
	...
	this.createBox = function() {
		var kk = $('<div id="debug_id1"><input type="button" class="btn" value="clear"/></div>');
		var kk2 = $('<div  id="debug_id2"></div>');
		...
		jQuery(this.Box).prepend(kk2);
		jQuery(this.Box).prepend(kk);
		kk.find("input").click(function(){
			kk2.empty();
		});
