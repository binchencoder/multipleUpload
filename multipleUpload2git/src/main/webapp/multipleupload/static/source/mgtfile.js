...
function MgtFileuploadPageClass() {
	...
	this.setResult = function(idoo,ok,size,server){
		var on = null,g1 = null;
		$.each(this.doms, function(k,v){
			$.each(v,function(k1,v1){
				if(v1.idoo==idoo){
		...
		if(idoo && idoo != null && idoo != ''){
			if(on){
				on.finish(size,ok,server);
				this.submit();
			}else if(g1){
				g1.finish(size,ok,server);
				this.submit();
			}
		}
	};
    ...
    $.fn.mgtfileupload = function(o) {
		...
		function createFile(){
			return {id : 0,//针对已经存在的附件
					...
					finish : function(size, ok, server){
...
//								log("in 009 " + this.formDiv.find("form").length   )
		...
		function createShowDiv(){
			...
			"<i class=\"mpic-delete2 ch\"></i><span><div style=\"display:none;width:100px;height:12px;margin-top:2px;\" class=\"progress progress-striped active\"><div class=\"bar\" style=\"width: 0%;\"></div></div></span>");
		    ...
		    if(!options.attached){
		    	...
		    	if(attsContainer.length == 0){
		    		attsContainer = $("<ul class='att_cont' id='" + attListDomId + "'></ul>"); 
		    	}
		    	self.append(attsContainer);
		    	
		    	var li = $("<li style='display:none'></li>");
		    	
		    	var attachItemDiv = $("<div style='display:none'></div>");
			    attachItemDiv.append("<span class=\"name\"></span><span style=\"margin:0 0 0 10px;\" class=\"attsize\"></span><span style=\"margin:0 0 0 10px;\" class=\"status\">等待</span>" +
			    ...
			    li.append(attachItemDiv);
			    attsContainer.append(li);
		    	
			    return attachItemDiv;
...
//				log(" in aFile.deleteme " );
				...
				options.onCancel && options.onCancel(aFile);
			});
		...
		var createNew = function(){
			...
			input.css({
				...
				'position':'absolute', 
				'font-size':'50px', 
				'right': ($.browser.msie || $.browser.webkit || ($.browser.mozilla && $.browser.version > 21.0))  ? '0px' : '426px', 
				'cursor':'pointer'
			});
...
//				log("in not msie ");
		...
		if(domId){
			...
			var files = options.attachedFiles;
			if(files){
