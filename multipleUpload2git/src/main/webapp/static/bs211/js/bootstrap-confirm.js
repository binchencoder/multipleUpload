...
!function( $ ){
	...
	Confirm.prototype = {
		...
		// 设置modal内容
		,setModalBody: function( ico, text ) {
			$(this.getModal()).find('.modal-body').html( "<p>" + ico + text + "</p>" );
		}
		// 显示与绑定
		,show: function() {
			if (typeof this.options.preAction == 'function' 
			...
			this.setModalBody( self._options.ico, self._options.message );
	...
	$.fn.confirm.defaults = {
		...
		ico: "<i class='mpic-mgtwarn me-margin-right10'></i>", // modal内容前面的提示图标
		template : '<div class="modal hide confirm-modal" data-keyboard="false" aria-hidden="false" data-backdrop="static"><div class="modal-header"><button class="close confirm-cancelbtn" data-dismiss="modal" type="button">×</button><h3 class="pull-left-ie"></h3></div><div class="modal-body"></div><div class="modal-footer"><a href="javascript:;" class="btn btn-primary confirm-btn">确定</a><a href="javascript:;" class="btn confirm-cancelbtn">取消</a></div></div>'
	}
